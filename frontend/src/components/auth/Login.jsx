...
export default function Login() {
    ...
    if (!emailTrimmed) erroresVista.email = "El email es obligatorio.";
    else if (!/\S+@\S+\.\S+/.test(emailTrimmed)) erroresVista.email = "El formato del email no es válido.";
    ...
    try {
        ...
        if (!response.ok) {
            try {
                ...
                setErroresGenerales([data.error || "Error al iniciar sesión."]);
            } catch {
                setErroresGenerales(["Error al iniciar sesión."]);
            } 
            return;
        }
        ...
        setErroresGenerales(["No se pudo conectar con el servidor."]);
    }
};


    const manejoCambioInput = (setter, campo) => (e) => {
